...
kernel void GaussJordanMain(global float* matrix, global float* I,  int n, queue_t que, int divPort, int elimPort, int elimPivPort)
{
	...
	for(int i=1;i<n;i++)
	{ 
		enqueue_kernel(que,CLK_ENQUEUE_FLAGS_NO_WAIT,divRange,1,&Events[4*i-1],&Events[4*i],^{GaussJordanDiv(matrix,I,n,i,divPort);});
		//release_event(Events[3]);
		enqueue_kernel(que,CLK_ENQUEUE_FLAGS_NO_WAIT,divDiagRange,1,&Events[4*i],&Events[4*i+1],^{GaussJordanDiagDiv(matrix,I,n,i);});
		//release_event(Events[0]);
		enqueue_kernel(que,CLK_ENQUEUE_FLAGS_NO_WAIT,elimRange,1,&Events[4*i+1],&Events[4*i+2],^{GaussJordanElim(matrix,I,n,i,elimPort);});
		//release_event(Events[1]);
		enqueue_kernel(que,CLK_ENQUEUE_FLAGS_NO_WAIT,elimRangePiv,1,&Events[4*i+2],&Events[4*i+3],^{GaussJordanElimPiv(matrix,I,n,i,elimPivPort);});
		//release_event(Events[2]);
	}
